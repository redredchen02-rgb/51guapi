...
- `packages/backend/` — Fastify 5 + TypeScript,端口 3002(无鉴权·个人自用、多渠道爬取/提炼管线、SSRF 守卫)
- `packages/shared/` — 跨端共享类型与纯逻辑(`@51guapi/shared`:facts、vocab、export、article-assembler、article-tags 等)
...
后端环境:复制 `packages/backend/.env.example` → `.env`。后端 **fail-closed**:`CORS_ORIGIN` 缺失或为 `*` 时拒绝启动。**完全无鉴权(v0.2.5)**：JWT 已整体移除,无 `/login`、无 token、无 preHandler。可爬取渠道白名单的写入也无口令/确认手势(默认种子仅 `51cg1.com`,UI 直接增删)。
...
- **SSRF 守卫**:爬取渠道走 `src/scraper/ssrf-guard.ts` 的 allowlist(fail-closed,列表为空全拒)+ 私有 IP 阻挡 + 输入层拒 IP literal。爬取管线/LLM 自身不可触发 allowlist 写入(防 prompt 注入自开渠道);新增渠道只需 UI 点击,无鉴权闸(plan 2026-06-18-003 + JWT 移除)
...
- 路由按模块分文件 `src/routes/*-routes.ts`,在 `index.ts` 统一 `register*Routes(server)`;无全局 preHandler(JWT 已移除),所有路由公开
...
- 扩展对后端的调用统一走 `apiFetch()`(plain Content-Type 头,无 Authorization)
