...
- [ ] 把以上值写入 `packages/backend/.env`（不入库；自用模式免密登入,无需 `JWT_ADMIN_PASSWORD_HASH`）
...
- [ ] 确认所有测试和构建通过
- [ ] 确认 `CHANGELOG.md` 已更新至 `[0.2.3]`
- [ ] 所有变更已合并到 main 分支

```bash
git tag v0.2.3
git push origin v0.2.3      # 触发 .github/workflows/release.yml
```

- [ ] GitHub Actions `release.yml` 触发，等待完成
- [ ] GitHub Releases 页有 v0.2.3，附件 `chrome-mv3.zip` 存在

---

## 阶段四：首次冒烟后操作

- [ ] 更新 `.ai-memory/project_51guapi.md`，记录首次成功使用的日期和关键数据
...
- 历史首飞 Runbook（旧发布流程归档，不作为当前操作参考）：`docs/runbooks/first-flight-runbook.md`
