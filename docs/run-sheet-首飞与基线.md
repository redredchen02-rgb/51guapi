...
> ⚠️ **已废弃（2026-06-16）**：本运行单描述「填入后台发布 + acgs51 漫画来源」的首飞流程。
> 该发布机器与 acgs51 来源已在 v0.1（吃瓜小幫手）转型中整体移除，下文 `ACGS51_*` 环境变量、
> `51acgs.com`、`/scraper/trigger/acgs51` 等均不再适用。保留仅作历史记录。
>
...
# 启动日志应显示：Server listening on http://127.0.0.1:3002
...
| 后端运行 | `curl http://127.0.0.1:3002/api/v1/auth/status` | `{"ok":true,"authenticated":false}` |
| 扩展已加载今天构建 | chrome://extensions → 查看「最后更新」时间 | 今天 |
| 登录 round-trip | 扩展侧边栏登录 | 200 + token |
| batch 写入 | `curl -X POST http://127.0.0.1:3002/api/v1/batches -H 'Authorization: Bearer <token>' -d '{"id":"smoke","tabId":1,"authorizedHost":"test","topics":["test"]}'` | `{"ok":true,...}` |
| pending 读取 | `curl http://127.0.0.1:3002/api/v1/pending-topics -H 'Authorization: Bearer <token>'` | `{"ok":true,"topics":[...]}` |
...
- **方式 2·curl(指定任意详情页,过 allowlist)**:
  ```bash
  curl -X POST http://127.0.0.1:3002/api/v1/scraper/trigger/acgs51 \
...
仅当需要在抓取不可用时演练后续流程:
```bash
curl -X POST http://127.0.0.1:3002/api/v1/pending-topics \
  -H 'Authorization: Bearer <token>' \
  -H 'Content-Type: application/json' \
...
按**首飞中止协议**(见 Part 1 之后的专节)执行:关回 ACGS51_ENABLED → 记录中止原因 → 在 Part 4 完成声明区写入带日期的改期承诺,同步到 `.ai-memory/project_51guapi.md`。U13 CORS 实测随首飞延期。
...
```bash
curl -X PATCH http://127.0.0.1:3002/api/v1/pending-topics/<id> \
  -H 'Authorization: Bearer <token>' \
  -H 'Content-Type: application/json' \
  -d '{"status":"pending"}'
```
