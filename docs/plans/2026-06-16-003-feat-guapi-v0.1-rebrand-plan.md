...
3. **改名**：`@51publisher` → `@51guapi`、品牌字串 → 吃瓜小幫手、版本歸零 `0.1.0`。
...
現有 `51publisher 發帖填充助手` 核心是把 AI 草稿填入第三方後台發帖。產品方向已變：不再發布，要的是持續從多個目標站爬取資源、AI 提煉成吃瓜草稿、導出自用。代碼庫已半路轉型（`gossip-*` 骨架已存在且穩定），但仍夾帶發布機器與漫畫來源兩條舊尾巴，品牌仍是「51publisher/發帖」。（see origin: docs/brainstorms/2026-06-16-guapi-v0.1-rebrand-requirements.md）

## Requirements Trace

- R1. 顯示名/描述/README/UI 文案改「吃瓜小幫手」，清除「51publisher / 發帖」。→ U5
- R2. npm scope `@51publisher/*` → `@51guapi/*`，import / package.json deps / tsconfig paths 同步。→ U4
- R3. 三包版本統一 `0.1.0`。→ U5
...
**Approach:**
- 先刪 entrypoints + UI，再刪其專屬 lib，最後改 `background.ts` / `messaging.ts` / `storage.ts` 收尾懸空引用，`pnpm --filter publisher-fill-assistant compile` 逐步逼綠。
...
**Test scenarios:**
- 整理：刪除上列發布測試後，剩餘擴展單測/e2e 全綠（`pnpm --filter publisher-fill-assistant test`）。
- Edge case：`background.ts` 收到已刪除的發布消息類型時，不應崩潰（無對應 handler 即忽略/回錯，不拋未捕獲異常）。
- 回歸：生成路徑（`GENERATE_DRAFT`）與 `GET_BATCH` 只讀仍可用。

**Verification:** `pnpm --filter publisher-fill-assistant compile` 與 `test` 全綠；代碼庫搜不到 `fillers` / `safety-gate` / `batch-orchestrator` / `first-flight` 引用。
...
**Approach:** 先改 shared `types.ts`/`index.ts` 並 `pnpm --filter @51publisher/shared build`，再讓 backend compile 暴露所有引用點逐一清理。

**Patterns to follow:** 路由註冊集中在 `index.ts` 的 `register*Routes(server)`；`PUBLIC_ROUTES` 白名單若含已刪路由須同步移除。

**Test scenarios:**
- 整理：後端單測全綠（`pnpm --filter publisher-backend test`），JWT preHandler 與保留路由不受影響。
...
- [ ] **Unit 4: npm scope 改名 `@51publisher` → `@51guapi`**
...
**Files:**
- Modify：`packages/shared/package.json`（`name`: `@51guapi/shared`）、`packages/backend/package.json` + `packages/extension/package.json`（deps `@51publisher/*` → `@51guapi/*`）、根 `package.json`、各 `tsconfig.json` 的 `paths` 別名
- Modify：所有 `import ... from "@51publisher/shared"` → `@51guapi/shared`（backend ~20 檔、extension 刪後剩餘檔）

**Approach:** 全庫搜替 `@51publisher` → `@51guapi`；先 `pnpm --filter @51guapi/shared build` 產出 dist，再 `pnpm -r compile`。`pnpm install` 重建 workspace 連結。
...
- Edge case：`grep -r "@51publisher" packages` 無命中（archive 文檔除外）；`pnpm install` 無 unmet workspace 依賴。
...
**Files:**
- Modify：`packages/extension/wxt.config.ts`（manifest `name`/`description` → 吃瓜小幫手）、`README.md`、`CLAUDE.md`/`AGENTS.md`（品牌詞 + build order 說明中的包名）、各 `package.json` `version` → `0.1.0`、UI 文案中含「51publisher/發帖」的字串（`App.tsx`、`Settings.tsx`、`AuthView.tsx` 等）
- Modify：代碼注釋中的品牌詞（非 archive）

**Approach:** 搜替 `51publisher` / `發帖填充助手` / `發帖` → 吃瓜小幫手語境；版本字段三包對齊 `0.1.0`。archive 歷史 plan 不動。

**Test scenarios:**
- Test expectation：none — 文案/版本變更無行為邏輯；通過判據為 compile 綠 + 人工掃 UI 無殘留舊品牌。
- Edge case：擴展載入後側邊欄標題顯示「吃瓜小幫手」；`grep -ri "51publisher\|發帖" packages --include=*.ts --include=*.tsx --include=*.json` 僅剩可接受殘留（如歷史注釋）。
...
- ⚠️ 不覆寫舊 `51publisher` 遠端已合併 PR；新倉是獨立起點。CI workflow 內的倉名/scope 引用同步更新。
...
**待辦（低優先，未做）**：
- 內部包名 `publisher-monorepo` / `publisher-backend` / `publisher-fill-assistant` 仍含 "publisher"（非 scope、非用戶可見）；改動會牽動 CI/腳本/命令的 `--filter` 引用，價值低，留待後續。
