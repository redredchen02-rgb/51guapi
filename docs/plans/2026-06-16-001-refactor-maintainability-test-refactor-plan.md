...
**Verification:**
- `pnpm --filter 51guapi-backend test` 通過，prompt-routes.test.ts 出現於通過清單，無 skipped
...
**Verification:**
- `pnpm --filter 51guapi-extension test` 通過，`chrome-storage-utils.test.ts` 出現於通過清單
...
**Verification:**
- `pnpm -r compile` 無報錯
- `pnpm --filter 51guapi-backend test` 全綠（所有 routes 測試通過）
...
- `pnpm --filter 51guapi-extension test` 全綠（含 `useTodayBatchDomain.test.ts`）
