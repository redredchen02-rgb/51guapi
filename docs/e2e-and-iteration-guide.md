...
> ⚠️ **已废弃（2026-06-22）**：本文描述旧版后台填充 / Quill fixture / 零提交 e2e 维护流程。
> 当前版本已删除填充与发布链路，不再维护 `lib/fillers.ts`、字段映射填充器或真后台 fixture 流程。
> 当前验证入口以 `scripts/check-all.sh`、`pnpm preflight` 和人工扩展加载 / 抓取 / 导出冒烟为准。
