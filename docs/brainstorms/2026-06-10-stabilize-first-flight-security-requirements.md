...
51guapi 产品功能推进很快（0.2.0：自动抓取管线、标签约束、封面图、待审池均已实现），但 2026-06-10 实测发现工程底盘处于「三重不稳定」（下列数字为 2026-06-10 上午快照，工作树仍在被并发修改，规划首步必须重测基线）：
...
- 止血：rescue/WIP 快照分支经密钥扫描后先行推送（R0）；fresh clone 后 `pnpm install && pnpm --filter @51guapi/shared build && pnpm -r compile && pnpm -r test` 全绿（含 R13 扩展侧 tsc 修复）；`git status` 干净（除合理忽略项）；远端分支可独立还原构建。
