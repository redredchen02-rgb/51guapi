...
**品牌重命名（Rename）**
- R1. 擴展顯示名與描述改為「吃瓜小幫手」，移除所有「51guapi / 發帖填充助手 / 發帖」字樣（manifest、README、UI 文案、面向用戶可見字串）。
...
**倉庫（Repository）**
- R14. ✅ 已建立新 repo `redredchen02-rgb/51guapi`（public，空倉），作為 v0.1 正式起點（與舊 `51guapi` repo 切割）。改造完成後推首版代碼。

## Success Criteria

- 全代碼庫（面向用戶可見處 + 套件名）搜不到「51guapi / 發帖」字樣；顯示名為「吃瓜小幫手」。
