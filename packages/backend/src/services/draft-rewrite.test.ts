...
describe("rewriteDraftLlm — A5 纯散文中和(P0,不依赖任何客户端允许集)", () => {
	...
	// omit-path 回归(对抗审计 P0):rewrite 仅针对部分维度时模型常省略其余字段;此前中和被
	// 条件化在「模型是否返回该字段」上,模型省略时客户端原始脏字段原样穿透进导出 sink。
	it("模型省略 body(只重写 title)→ 客户端原 draft.body 裸文本链接仍被无条件中和", async () => {
		const res = await rewrite(
			{ title: "新标题" }, // 模型不返回 body
			{ ...draft, body: `<p>详情见 ${EVIL} 速看</p>` },
		);
		expect(res.ok).toBe(true);
		if (res.ok) {
			expect(res.draft.body).not.toContain("evil.example.net");
	...
	it("模型省略 title(只重写 body)→ 客户端原 draft.title 链接仍被中和", async () => {
		const res = await rewrite(
			{ body: "<p>正文</p>" }, // 模型不返回 title
			{ ...draft, title: `标题 ${EVIL}` },
		);
		expect(res.ok).toBe(true);
		if (res.ok) expect(res.draft.title).not.toContain("evil.example.net");
	...
	it("模型省略 tags → 客户端原 draft.tags 链接仍被中和", async () => {
		const res = await rewrite(
			{ body: "<p>正文</p>" }, // 模型不返回 tags
			{ ...draft, tags: ["正常", EVIL] },
		);
		expect(res.ok).toBe(true);
		if (res.ok)
	...
	it("集成(真 sink,omit-path):模型只返回 title 时客户端脏 body 不进 JSON/Markdown 导出", async () => {
		const res = await rewrite(
			{ title: "新标题" },
			{ ...draft, body: `<p>看 <a href="${EVIL}">这</a> 或 ${EVIL}</p>` },
		);
		expect(res.ok).toBe(true);
		if (res.ok) {
