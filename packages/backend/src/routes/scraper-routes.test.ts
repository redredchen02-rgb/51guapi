...
vi.mock("../scraper/auto-generate.js", () => ({
	autoGenerateDrafts: vi.fn(),
}));
...
// ================================================================
// POST /api/v1/scraper/auto-generate — legacy='acg' branches
// ================================================================

import { autoGenerateDrafts } from "../scraper/auto-generate.js";

const mockAutoGenerate = vi.mocked(autoGenerateDrafts);

describe("POST /api/v1/scraper/auto-generate — legacy:acg 分支", () => {
	afterEach(() => {
		delete process.env.LLM_ENDPOINT;
		delete process.env.LLM_API_KEY;
	});

	it("legacy:acg + 缺少 LLM env → 500", async () => {
		const res = await app.inject({
			method: "POST",
			url: "/api/v1/scraper/auto-generate",
			payload: { legacy: "acg" },
	...
	it("legacy:acg + LLM env 完整 + autoGenerateDrafts 成功 → 200 ok=true", async () => {
		process.env.LLM_ENDPOINT = "https://llm.example.com/v1";
		process.env.LLM_API_KEY = "test-key";
		mockAutoGenerate.mockResolvedValueOnce({
			generated: 2,
			skipped: 1,
			errors: 0,
			drafts: [],
		...
		expect(res.statusCode).toBe(200);
		expect(res.json().ok).toBe(true);
		expect(res.json().generated).toBe(2);
		expect(res.json().progress.total).toBe(3);
	});

	it("legacy:acg + LLM env 完整 + autoGenerateDrafts 拋錯 → 500", async () => {
		process.env.LLM_ENDPOINT = "https://llm.example.com/v1";
		process.env.LLM_API_KEY = "test-key";
		mockAutoGenerate.mockRejectedValueOnce(new Error("DB error"));
